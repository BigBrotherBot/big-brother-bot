...
Source: "frostbite_server_info.exe"; DestDir: "{tmp}"; Flags: dontcopy
Source: "frostbite_server_info.bat"; DestDir: "{tmp}"; Flags: dontcopy
...
Filename: {app}\b3_run.exe; Parameters: "--config ""{app}\conf\b3.xml"""; WorkingDir: {app}; Flags: ShellExec PostInstall; Description: "Run Setup"; 
...
begin
  ...
  RconPage.Values[2] := GetPreviousData('RconPassword', '');
...
begin
  ...
  SetPreviousData(PreviousDataKey, 'RconPassword', RconPage.Values[2]);
end;
...
begin
  ...
  if not Exec(ExpandConstant('{tmp}\frostbite_server_info.bat'), ProgramParams, '', SW_SHOW, ewWaitUntilTerminated, ResultCode) then begin
    ...
    if Result then begin
      ...
      if not CompareStr(PasswordAccepted, 'True') = 0 then begin
      ...
      MsgBox('Error while checking server information' + #13#13 + GetIniString('general', 'error', 'unknown error', ServerInfoFilePath), mbError, MB_OK); 
    end;
  end;
