...
class EventManager(LoggerMixin, object):
    ...
    def create(self, name:str, description:str):
        """Dynamically create a new event type"""
        self.debug('Creating event %s (%s)', name, description)
        EventType.extend(name, description)
...
class EventRouter(object):
    """
    This class helps defining handlers functions to be called when a game event is received.
    This class provides:
      - A decorator for associating a regular expression to a function: @router.gameEvent(<regular expression>, ...)
      - A method handler(<text>) that return a tuple <func, dict> where:
          * func is the first handler function defined with the 'event' decorator that matches the given *text*
          * dict is a dictionary of all matched groups from the regular expression that was associated to *func*
            with the decorator.

    USAGE :
    -------
    This tool is meant to be used by B3 game parsers which need to parse game event (in the
    form of a string) and call a function that takes action for this type of game event.

    >>> from b3.events import EventRouter
    >>>
    >>> router = EventRouter()
    >>>
    ...
    >>> @router.gameEvent("^join: (?P<name>.+), (?P<team>.+)$")
    >>> def onConnect(self, name:str, team:str):
    ...
    The @router.gameEvent decorator accepts multiple parameters if you need to have one handling
    function for multiple kind of game events. Note that those regular expressions should all define
    the same groups.
    """
    def __init__(self):
        self._compiled_retype = type(re.compile(''))
        self._mappings = []

    def gameEvent(self, *params):
        def wrapper(func):
            for param in params:
                if isinstance(param, self._compiled_retype):
                    self._mappings.append((param, func))
                elif isinstance(param, str):
                    self._mappings.append((re.compile(param), func))
            return func
        return wrapper

    def handler(self, gameEvent:str):
        """Returns the (func, dict) matching the given game event"""
        for r, func in self._mappings:
            m = r.match(gameEvent)
            if m:
