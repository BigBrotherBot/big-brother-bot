...
# * rewrite method writelines so it does not lock until all lines have been sent. This allows other commands made
#   by another thread to be sent in between commands sent via writelines (and B3 won't appear to be unresponsive)
...
# * rollback changes made in 1.6 as it does not solve much (at least with UrT) and break some rcon commands
#   with at least cod6
# 2012/12/22 - 1.8 - courgette
# * do not strip data in game server responses
# 2014/04/14 - 1.8.1 - Fenix
...
class Rcon:
    ...
    def __init__(self, console, host, password):
        ...
        self.console.bot('rcon status Cache Expire Time: '
                         '[%s sec] Type: [%s]' % (self.status_cache_expire_time, self.status_cache_type))
    ...
    def encode_data(self, _data, source):
        try:
            if isinstance(_data, str):
                _data = unicode(_data, errors='ignore')
            _data = _data.encode(self.console.encoding, 'replace')
        except Exception, msg:
            self.console.warning('%s: ERROR encoding data: %r', source, msg)
            _data = 'Encoding error'
            
        return _data
        
    def send(self, _data, maxRetries=None, socketTimeout=None):
        ...
        if self.console.encoding:
            _data = self.encode_data(_data, 'QSERVER')

        self.console.verbose('QSERVER sending (%s:%s) %r', self.host[0], self.host[1], _data)
        ...
        while time.time() - start_time < 5:
            ...
            if len(errors) > 0:
                ...
                try:
                    writeables[0].send(self.qserversendstring % _data)
                except Exception, msg:
                    self.console.warning('QSERVER: ERROR sending: %r', msg)
                else:
                    try:
                        _data = self.readSocket(self.socket, socketTimeout=socketTimeout)
                        self.console.verbose2('QSERVER: Received %r' % _data)
                        return _data
                    except Exception, msg:
            ...
            if retries >= maxRetries:
                self.console.error('QSERVER: too much tries. Abording (%r)', _data.strip())
                break

            self.console.verbose('QSERVER: retry sending %r (%s/%s)...', _data.strip(), retries, maxRetries)

        self.console.debug('QSERVER: Did not send any data')
        return ''

    def sendRcon(self, _data, maxRetries=None, socketTimeout=None):
        if socketTimeout is None:
            socketTimeout = self.socket_timeout
        if maxRetries is None:
            maxRetries = 2
        ...
        # encode the data
        if self.console.encoding:
            _data = self.encode_data(_data, 'RCON')

        self.console.verbose('RCON sending (%s:%s) %r', self.host[0], self.host[1], _data)
        start_time = time.time()
        ...
        while time.time() - start_time < 5:
            ...
            if len(errors) > 0:
                ...
                try:
                    writeables[0].send(self.rconsendstring % (self.password, _data))
                except Exception, msg:
                    self.console.warning('RCON: ERROR sending: %r', msg)
                else:
                    try:
                        _data = self.readSocket(self.socket, socketTimeout=socketTimeout)
                        self.console.verbose2('RCON: Received %r' % _data)
                        return _data
                    except Exception, msg:
                        self.console.warning('RCON: ERROR reading: %r', msg)

                if re.match(r'^quit|map(_rotate)?.*', _data):
                    # do not retry quits and map changes since they prevent the server from responding
                    self.console.verbose2('RCON: no retry for %r', _data)
                    return ''
            ...
            if retries >= maxRetries:
                self.console.error('RCON: too much tries. Abording (%r)', _data.strip())
                break
            self.console.verbose('RCON: retry sending %r (%s/%s)...', _data.strip(), retries, maxRetries)
    ...
    def write(self, _cmd, maxRetries=None, socketTimeout=None, Cached=True):
        #intercept status request for caching construct
        if _cmd == 'status' and Cached:
            status_cache = self.cache.get_cache('status',
                                                type=self.status_cache_type,
    ...
    def readNonBlocking(self, sock):
        ...
        while time.time() - start_time < 1:
            try:
                ...
                if d:
                    # remove rcon header
                    _data += d.replace(self.rconreplystring, '')
                elif len(_data) > 0 and ord(_data[-1:]) == 10:
                    break
