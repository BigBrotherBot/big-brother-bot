...
import socket
import b3.parsers.frostbite2.protocol as protocol
...
class FrostbiteConnection(object):
    ...
    def __init__(self, console, host, port, password, name=None):
        self.console = console
        self._host = host
        self._port = port
        self._password = password
        self.name = name if name else hash(self) # useful when using multiple connections
    ...
    def sendRequest(self, *command):
        ...
        try:
            ...
            # FIXME: remove this once Frostbite2 admin.say command is bug free
            if words and words[0] == 'admin.say':
                # close connection to force reconnecting as admin.say breaks it
                self.close()
                return ['OK']
    ...
    def _auth(self):
        self.console.debug('authing to Frostbite2 server')
        if self._serverSocket is None:
            raise FrostbiteException("cannot auth, need to be connected")
        ...
        if loginResponse[0] != "OK":
            raise FrostbiteBadPasswordException("The Frostbite2 server refused our password")
